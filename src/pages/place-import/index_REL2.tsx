  ...
  const handleGenerate = () => {
    ...
    try {
      ...
      for (let i = 0; i < maxCount; i++) {
        ...
        let imageUrl = matchedImage;
        if (matchedImage) {
          // Если путь относительный, добавляем протокол и хост
          if (matchedImage.startsWith('/')) {
            imageUrl = `${window.location.protocol}//${window.location.host}${matchedImage}`;
          } else if (!matchedImage.startsWith('http')) {
            // Если это просто имя файла, формируем полный путь
            imageUrl = `${window.location.protocol}//${window.location.host}/storage/uploads/${matchedImage}`;
          }
        }
                  ...
                  <div className="relative w-full h-32 bg-gray-200 rounded overflow-hidden">
                    {place.image ? (
                      <>
                        ...
                        />
                        {/* Placeholder */}
                        <div className="image-placeholder absolute inset-0 flex items-center justify-center text-gray-400 text-xs bg-gray-100">
                          <div className="text-center">
                            <div className="w-6 h-6 bg-gray-300 rounded mx-auto mb-1"></div>
                            {place.imageName ? place.imageName.split('/').pop()?.substring(0, 15) + '...' : 'Фото'}
                          </div>
                        </div>
                      </>
                    ) : (
