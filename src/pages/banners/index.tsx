...
export default function BannersPage() {
  ...
  const load = () =>
    HttpClient.get<any>('/api/admin/homepage-banners')
      .then((d) => {
        setRows(d.banners || []);
        setAutoplay(d.autoplay);
        setIntervalValue(d.interval_ms);
      })
      .catch((e: any) =>
        toast.error(
  ...
  const saveSettings = async () => {
    await HttpClient.post('/api/admin/homepage-banners/settings', {
      autoplay,
  ...
  const remove = async (id: number) => {
    ...
    await HttpClient.delete(`/api/admin/homepage-banners/${id}`);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-body">
              Сначала выберите баннер из списка или создайте новый.
            </p>
          </div>
      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left">
            <thead>
              <tr className="border-b text-sm text-body">
                ...
                <tr key={b.id} className="border-b last:border-0">
                  ...
                  <td className="p-4">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() => remove(b.id)}
                        className="rounded border border-red-200 px-3 py-2 font-semibold text-red-600"
                      >
                        Удалить
                      </button>
                    </div>
          ...
          {loading && (
            <div className="p-8 text-center text-body">Загрузка…</div>
          )}
          {!loading && !rows.length && (
            <div className="p-8 text-center text-body">Баннеров пока нет.</div>
          )}
        </div>
      </Card>
    </div>
