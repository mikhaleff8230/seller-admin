...
type Settings = { enabled:boolean; oauth_token:string; oauth_token_configured:boolean; client_login:string; campaign_id:string; feed_id:string; markup_percent:string; balance_reserve:string; sync_interval_minutes:number; campaign_bid_ceiling:string; default_bid_level:string; allowed_bid_levels:number[] };
const defaults: Settings = {enabled:false,oauth_token:'',oauth_token_configured:false,client_login:'',campaign_id:'',feed_id:'',markup_percent:'30',balance_reserve:'100',sync_interval_minutes:15,campaign_bid_ceiling:'40',default_bid_level:'20',allowed_bid_levels:[5,10,15,20,30,40]};
...
export default function YandexDirectPage(){
 ...
 const syncStrategy=async()=>{setBusy(true);try{const d=await HttpClient.post<any>('/api/admin/yandex-direct/sync-strategy',{});toast.success(`Потолок ${d.strategy?.bid_ceiling} ₽ применён, недельный бюджет ${d.strategy?.weekly_budget} ₽ сохранён`);await test()}catch(e:any){toast.error(e?.response?.data?.message||'Стратегия не синхронизирована')}finally{setBusy(false)}};
 ...
 return <div className="space-y-6">
  ...
  <Card><div className="grid gap-5 md:grid-cols-2">
   ...
   <label>Максимальная интенсивность ЕПК, ₽<input type="number" min="1" value={settings.campaign_bid_ceiling} onChange={e=>field('campaign_bid_ceiling',e.target.value)} className={input}/></label>
   <label>Уровень по умолчанию, ₽<input type="number" min="1" value={settings.default_bid_level} onChange={e=>field('default_bid_level',e.target.value)} className={input}/></label>
   <label className="md:col-span-2">Разрешённые уровни seller, ₽<input value={settings.allowed_bid_levels.join(', ')} onChange={e=>field('allowed_bid_levels',e.target.value.split(',').map(v=>Number(v.trim())).filter(Boolean))} placeholder="5, 10, 15, 20, 30, 40" className={input}/></label>
  </div><div className="mt-6 flex flex-wrap gap-3"><Button loading={busy} onClick={save}>Сохранить</Button><Button loading={busy} variant="outline" onClick={test}>Проверить подключение</Button><Button loading={busy} variant="outline" onClick={syncStrategy}>Применить потолок к ЕПК</Button></div>
  {check&&<div className={`mt-5 rounded p-4 ${check.success?'bg-green-50 text-green-800':'bg-red-50 text-red-700'}`}>{check.success?<>✓ API подключен<br/>✓ ЕПК: {check.campaign?.name} #{check.campaign?.id}<br/>✓ Feed #{check.feed?.id}<br/>Стратегия поиска: {check.strategy?.search_type||'—'} · потолок {check.strategy?.search_bid_ceiling??'—'} ₽<br/>Стратегия сетей: {check.strategy?.network_type||'—'} · потолок {check.strategy?.network_bid_ceiling??'—'} ₽</>:check.message}</div>}</Card>
  <Card><h2 className="mb-4 text-lg font-bold">Статус интеграции</h2><div className="grid gap-4 md:grid-cols-3"><Stat label="Активных seller groups" value={monitor.active_seller_groups||0}/><Stat label="Активных Boost товаров" value={monitor.active_boost_products||0}/><Stat label="Расход Яндекс сегодня" value={`${monitor.yandex_cost_today||0} ₽`}/><Stat label="Списано seller" value={`${monitor.seller_charged_today||0} ₽`}/><Stat label="Маржа SANCAN" value={`${Math.max(0,(+monitor.seller_charged_today||0)-(+monitor.yandex_cost_today||0)).toFixed(2)} ₽`}/><Stat label="Последняя синхронизация" value={monitor.last_sync_at||'—'}/></div>{monitor.intensity_distribution?.length>0&&<div className="mt-6"><h3 className="mb-3 font-semibold">Выбранная интенсивность seller</h3><div className="flex flex-wrap gap-3">{monitor.intensity_distribution.map((row:any)=><div key={row.bid_level} className="rounded bg-gray-50 px-4 py-3"><b>{Number(row.bid_level)} ₽</b><span className="ml-2 text-body">{row.sellers} seller</span></div>)}</div></div>}</Card>
