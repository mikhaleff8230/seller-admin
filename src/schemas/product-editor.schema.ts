...
export const ProductEditorSchema = z.object({
  ...
  group_variants: z.preprocess((val: unknown) => normalizeArray(val, []), z.array(z.object({
    ...
    media_order: z.array(z.string()).optional(),
  }))),
  ...
  media_order: z.preprocess((val: unknown) => normalizeArray<string>(val, []), z.array(z.string())),
...
export const MediaStepSchema = z.object({
  ...
  media_order: z.array(z.string()).optional().default([]),
  video: VideoFileSchema.optional().nullable(),
  video_as_cover: z.boolean().optional().default(false),
  remove_video: z.boolean().optional().default(false),
});
