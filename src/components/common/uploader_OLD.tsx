...
export default function Uploader({
  ...
  const thumbs = files?.map((file: any, idx) => {
    ...
    if (file && (file.id || file.thumbnail || file.original)) {
      ...
      let splitArray, fileType, filename, isImage, isVideo;
      
      if (file?.file_name) {
        splitArray = file.file_name.split('.');
        fileType = splitArray?.pop();
        filename = splitArray?.join('.');
        isImage = imgTypes.includes(fileType?.toLowerCase());
        isVideo = videoTypes.includes(fileType?.toLowerCase());
      } else if (file?.thumbnail) {
        // Извлекаем расширение из URL
        const urlParts = file.thumbnail.split('.');
        fileType = urlParts[urlParts.length - 1]?.split('?')[0]; // убираем query параметры
        filename = `file_${idx}`;
        isImage = fileType && imgTypes.includes(fileType.toLowerCase());
        isVideo = fileType && videoTypes.includes(fileType.toLowerCase());
        ...
        fileType = acceptVideo ? 'mp4' : 'jpg'; // по умолчанию
        filename = `file_${idx}`;
        isImage = !acceptVideo;
        isVideo = acceptVideo;
      }
