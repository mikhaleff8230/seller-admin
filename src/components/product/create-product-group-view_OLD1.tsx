    ...
    const loadProducts = async () => {
      ...
      if (isEditMode && existingGroupKey) {
        try {
          ...
          const groupProducts = response?.data || [];
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {isEditMode 
                ? 'Используется системой для связи товаров в группе. Не может быть изменен при редактировании.'
      ...
      <div className="flex justify-end space-x-3 pt-4 border-t">
        ...
        >
          {isSaving 
            ...
            : (isEditMode 
              ? `Сохранить изменения (${products.length} товаров)` 
              : `Создать группу (${products.length} товаров)`)
          }
        </Button>
      </div>
    </Card>
